cat << 'EOF' > app.py && git add app.py && git commit -m "Kesin Düzeltme: BIST Filtresi ve Belirgin Metinler" && git push origin main
...
<style>
    ...
    .hero-title { font-size: 4.5rem; font-weight: 900; color: #ffffff !important; margin-bottom: 10px; letter-spacing: -2px; }
...
def fetch_assets(query):
    ...
    try:
        ...
        return [{"label": f"🇹🇷 {q.get('shortname')} ({q.get('symbol')})" if q in bist else f"🌍 {q.get('shortname')} ({q.get('symbol')})", 
                 "symbol": q.get('symbol'), "name": q.get('shortname')} for q in sorted_list[:8]]
    except: return []
...
if user_input:
    ...
    if results:
        ...
        selected_ticker = [r['symbol'] for r in results if r['label'] == choice][0]
        df = yf.download(selected_ticker, period="1y", progress=False)['Close']
        if not df.empty:
